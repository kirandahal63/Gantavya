		...
		<div class="main-panel">
			<header class="header-nav">
				<div class="header-left">
					...
					<p class="breadcrumb">
						Admin / <span class="active-crumb">Routes</span>
					</p>
				</div>
			</header>
			...
			<div class="content-wrapper">
				...
				<div id="successToast" class="success-toast">
					<i class="fa-solid fa-circle-check"></i> <span> <%=request.getParameter("message").equals("RouteAdded") ? "New Route added successfully!" : ""%>
						<%=request.getParameter("message").equals("RouteUpdated") ? "Route details updated successfully!" : ""%>
					</span>
				</div>
				<script>
					            setTimeout(() => {
							...
							<div class="input-grid">
								<div class="input-group">
									<label>Route Name</label> <input type="text" name="routeName"
										value="${errorRoute != null ? errorRoute.routeName : ''}"
										required>
								</div>
								<div class="input-group">
									<label>Origin</label> <input type="text" name="origin"
										value="${errorRoute != null ? errorRoute.origin : ''}"
										required>
								</div>
								<div class="input-group">
									<label>Destination</label> <input type="text"
										name="destination"
										value="${errorRoute != null ? errorRoute.destination : ''}"
										required>
								</div>
								<div class="input-group">
									<label>Distance (KM)</label> <input type="text" name="distance"
										value="${distanceError != null ? '' : (errorRoute != null ? errorRoute.distance : '')}"
										placeholder="${distanceError != null ? distanceError : ''}"
										style="${distanceError != null ? 'border-color: red;' : ''}"
										required>
								</div>
								...
								<div class="btn-group-row" style="display: flex; gap: 15px;">
									...
									<button type="button" class="btn-secondary"
										onclick="window.location.href='route'">Cancel</button>
								</div>
						...
						<div class="glass-card modal-content" style="max-width: 650px;">
							...
							<form action="route" method="POST">
								<input type="hidden" name="action" value="update"> <input
									type="hidden" name="routeId" value="${editableRoute.routeId}">

								<!-- Use grid instead of flex for the 2-column look -->
								<div class="input-grid-modal">
									<div class="input-group">
										<label>Route Name</label> <input type="text" name="routeName"
											value="${editableRoute.routeName}" required>
									</div>
									<div class="input-group">
										<label>Origin</label> <input type="text" name="origin"
											value="${editableRoute.origin}" required>
									</div>
									<div class="input-group">
										<label>Destination</label> <input type="text"
											name="destination" value="${editableRoute.destination}"
											required>
									</div>
									<div class="input-group">
										<label>Distance</label> <input type="text" name="distance"
											value="${distanceError != null ? '' : editableRoute.distance}"
											placeholder="${distanceError != null ? distanceError : ''}"
											style="${distanceError != null ? 'border-color: red;' : ''}"
											required>
									</div>
								...
								<tr>
									...
									<td>
										<div class="action-icons">
											<a href="route?action=edit&id=${route.routeId}"
												class="edit-link"><i class="fa-solid fa-pen"></i></a> <a
												href="route?action=delete&id=${route.routeId}"
												class="delete-link"
